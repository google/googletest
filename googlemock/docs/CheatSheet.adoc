...
The typical flow is:
 ...
 1. Exercise code that uses the mock objects; if necessary, check the result using link:../../googletest/[Google Test] assertions.
...
link:../../googletest/[Google Test]). They
...
syntax defined
link:../../googletest/docs/advanced.adoc#regular-expression-syntax[here].
...
.. an array passed as a pointer and a count (e.g. in `Bar(const T* buffer, int len)` – see <<multiargument-matchers,Multi-argument Matchers>>).
...
|`SafeMatcherCast&lt;T&gt;(m)` |link:CookBook.adoc#casting-matchers[safely casts] matcher `m` to type `Matcher&lt;T&gt;`.
...
. The matcher body must be _purely functional_ (i.e. it cannot have any side effect, and the result must not depend on anything other than the value being matched and the matcher parameters).
...
|`ASSERT_THAT(expression, m)` |Generates a link:../../googletest/docs/primer.adoc#assertions[fatal failure] if the value of `expression` doesn't match matcher `m`.
...
See this link:CookBook.adoc#using-check-points[recipe] for one application of it.
