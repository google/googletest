...
. If `TestCaseName` starts with an `_` followed by an upper-case letter (say,
`_Foo`), we end up with `_Foo_TestName_Test`, which is reserved and thus
invalid.
. If `TestCaseName` ends with an `_` (say, `Foo_`), we get
...
Please read link:advanced.adoc#death-test-styles[Fixing Failing Death Tests] for what you can
do.
...
In particular, if you use link:../../googlemock[gMock] and the death test statement
...
For a complete example using derived test fixtures, see https://github.com/google/googletest/blob/master/googletest/samples/sample5_unittest.cc[googletest
sample]
...
link:advanced.adoc#assertion-placement[here].

== My death test hangs (or seg-faults). How do I fix it?

In googletest, death tests are run in a child process and the way they work is
delicate. To write death tests you really need to understand how they work.
Please make sure you have read link:advanced.adoc#how-it-works[this].

In particular, death tests don't like having multiple threads in the parent
process. So the first thing you can try is to eliminate creating threads outside
of `EXPECT_DEATH()`. For example, you may want to use link:../../googlemock[mocks]
...
link:advanced.adoc#assertion-placement[section] in the user's guide explains it.
...
needs to be defined in the _same_ name space. See go/totw/49 for details.
...
https://en.wikipedia.org/wiki/Heisenbug[Heisenbugs]. Therefore we strongly
advise against the practice, and googletest doesn't provide a way to do it.

In general, the recommended way to cause the code to behave differently under
test is https://en.wikipedia.org/wiki/Dependency_injection[Dependency Injection]. You can inject
different functionality from the test and from the production code. Since your
production code doesn't link in the for-test logic at all (the
https://docs.bazel.build/versions/master/be/common-definitions.html#common.testonly[`testonly`]
